...
def enrich(ip):
    ...
    if is_bad:
        log.warning(f"🚨 KNOWN BAD | {ip} | Score:{score}/100 | "
                    f"OTX:{otx.get('pulses',0)} | {abuse.get('country','')} | {abuse.get('isp','')}")
    else:
        log.info(f"✅ Clean | {ip} | Score:{score}/100 | {abuse.get('country','')} | OTX:{otx.get('pulses',0)}")

    producer.produce("threat-intel", json.dumps(result).encode(), callback=delivery_report)
    producer.poll(0)
    return result

def main():
    log.info("🚀 Threat Intel Enricher V1 starting...")
    r.ping()
        ...
        while True:
            ...
            try:
                ...
                for field in ("Source IP","src_ip","Destination IP","dst_ip"):
                    ...
                    if ip and ip not in seen and not is_private(ip):
                        enrich(ip)
                        seen.add(ip)
                        if len(seen) > 2000:
                            seen.clear()
